  ...
  if (currentConfig && manager.name === 'NPM') {
    ...
    var
      ...
      updatePaths = {
        ...
        defaultTheme: path.join(currentConfig.paths.source.themes,
    ...
    if (fs.existsSync(updatePaths.definition)) {
      ...
      if (currentConfig.version !== release.version) {
        console.log(
          'Updating Semantic UI from ' + currentConfig.version + ' to '
          + release.version);

        console.info('Updating ui definitions...');
        wrench.copyDirSyncRecursive(source.definitions, updatePaths.definition,
          settings.wrench.overwrite);

        console.info('Updating default theme...');
        wrench.copyDirSyncRecursive(source.themes, updatePaths.theme,
          settings.wrench.merge);
        wrench.copyDirSyncRecursive(source.defaultTheme,
          updatePaths.defaultTheme, settings.wrench.overwrite);

        console.info('Updating tasks...');
        wrench.copyDirSyncRecursive(source.tasks, updatePaths.tasks,
          settings.wrench.overwrite);
        ...
        wrench.copyDirSyncRecursive(source.site, updatePaths.site,
          settings.wrench.merge);
      ...
      console.error('Cannot locate files to update at path: ',
...
// PM that supports Build Tools (NPM Only Now)
  ...
  gulp.task('create install files', function (callback) {
    ...
    if (manager && (answers.useRoot || answers.customRoot)) {
      ...
      for (var destination in installPaths) {
        if (installPaths.hasOwnProperty(destination)) {
          ...
          installPaths[destination] = (destination == 'config' || destination
            == 'configFolder')
      ...
      wrench.copyDirSyncRecursive(source.definitions, installPaths.definition,
        settings.wrench.overwrite);

      console.info('Copying UI themes');
      wrench.copyDirSyncRecursive(source.themes, installPaths.theme,
        settings.wrench.merge);
      wrench.copyDirSyncRecursive(source.defaultTheme,
        installPaths.defaultTheme, settings.wrench.overwrite);

      console.info('Copying gulp tasks');
      wrench.copyDirSyncRecursive(source.tasks, installPaths.tasks,
        settings.wrench.overwrite);
    ...
    if (fs.existsSync(installPaths.site)) {
      console.info('Site folder exists, merging files (no overwrite)',
        installPaths.site);
    } else {
      console.info('Creating site theme folder', installPaths.site);
    }
    wrench.copyDirSyncRecursive(source.site, installPaths.site,
      settings.wrench.merge);
    ...
    gulp.task('create theme.config', function () {
      var
        ...
        pathToSite = path.relative(path.resolve(installPaths.themeConfigFolder),
          path.resolve(installPaths.site)).replace(/\\/g, '/'),
