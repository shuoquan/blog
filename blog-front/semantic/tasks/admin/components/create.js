  ...
  for (index in release.components) {
    ...
    (function (component) {
      ...
      var
        ...
        capitalizedComponent = component.charAt(0).toUpperCase()
          + component.slice(1),
      ...
      gulp.task(task.npm, false, function () {
        ...
        .pipe(flatten())
        .pipe(
          replace(regExp.match.componentExport, regExp.replace.componentExport))
        .pipe(replace(regExp.match.componentReference,
          regExp.replace.componentReference))
        .pipe(
          replace(regExp.match.settingsExport, regExp.replace.settingsExport))
        .pipe(replace(regExp.match.settingsReference,
      ...
      gulp.task(task.notes, false, function () {
        ...
        .pipe(
          replace(regExp.match.unrelatedNotes, regExp.replace.unrelatedNotes))
        .pipe(replace(regExp.match.whitespace, regExp.replace.whitespace))
        .pipe(
          replace(regExp.match.spacedVersions, regExp.replace.spacedVersions))
      ...
      gulp.task(task.meteor, function () {
        ...
        .on('end', function () {
          ...
          .on('end', function () {
            ...
            filenames = filenames.replace(regExp.match.trailingComma,
              '').trim();
